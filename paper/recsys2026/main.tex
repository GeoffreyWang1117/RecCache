...
%----------------------------------------------------------------------
\subsection{Multi-Cluster Speculation (S1)}
\label{sec:exp_s1}
%----------------------------------------------------------------------

\begin{table}[t]
\centering
\caption{Multi-cluster speculation ($K$ sweep, score-ratio $\tau$=0.35).}
\label{tab:s1}
\small
...
%----------------------------------------------------------------------
\subsection{Acceptance Criterion Comparison (S2)}
\label{sec:exp_s2}
%----------------------------------------------------------------------

\begin{table}[t]
\centering
\caption{Acceptance criterion comparison ($K$=3). Only score-ratio enables MCG.}
\label{tab:s2}
\small
...
\multirow{3}{*}{ML-1M}
& Cosine & .093 & 99.4\% & 0\% \\
& ScoreRatio & .090 & 97.0\% & \textbf{55.1\%} \\
& Heuristic & .100 & 77.4\% & 0\% \\
\midrule
\multirow{3}{*}{A-Elec.}
& Cosine & .006 & 99.8\% & 0\% \\
& ScoreRatio & .004 & 71.0\% & \textbf{54.4\%} \\
& Heuristic & .007 & 9.0\% & 0\% \\
\bottomrule
\end{tabular}
\end{table}
...
%----------------------------------------------------------------------
\subsection{Quality--Speed Pareto Front (S3)}
\label{sec:exp_s3}
%----------------------------------------------------------------------

\begin{table}[t]
\centering
\caption{Pareto front on ML-1M (score-ratio, $K$=3). Fresh NDCG: 0.165.}
\label{tab:s3}
\small
...
%----------------------------------------------------------------------
\subsection{Pool Retrieval vs Static Cache (S5)}
\label{sec:exp_s5}
%----------------------------------------------------------------------

\begin{table}[t]
\centering
\caption{Pool retrieval vs static cache ($K$=3, $\tau$=0.35). Pool consistently improves coverage 1.1--3.9$\times$. On Electronics, pool approaches fresh-model NDCG.}
\label{tab:s5}
\small
\begin{tabular}{ll cccc}
\toprule
...
%----------------------------------------------------------------------
\subsection{End-to-End Comparison (S4)}
\label{sec:exp_s4}
%----------------------------------------------------------------------

\begin{table}[t]
\centering
\caption{End-to-end comparison. Best cached NDCG in \textbf{bold}; best cached coverage \underline{underlined}. $\pm$std from 3 runs.}
\label{tab:s4}
\small
\begin{tabular}{ll ccccc}
\toprule
...
& Sp.+Pool & .094 & 93\% & 0.7$\times$ & 60\% & \underline{.062} \\
\midrule
...
& Sp.+Pool & .007 & 60\% & 2.0$\times$ & 61\% & \underline{.074} \\
\midrule
...
& Sp.+Pool & .004 & 96\% & 3.6$\times$ & 62\% & \underline{.036} \\
\midrule
...
& Sp.+Pool & \textbf{.014} & 26\% & 2.1$\times$ & 49\% & \underline{.106} \\
\bottomrule
\end{tabular}
\end{table}
...
%----------------------------------------------------------------------
\subsection{Clustering Quality (S6)}
\label{sec:exp_s6}
%----------------------------------------------------------------------

\begin{table}[t]
\centering
\caption{Semantic vs random clustering. Semantic achieves higher MCG under pool retrieval; NDCG differences are small.}
\label{tab:s6}
\small
\begin{tabular}{ll cccc}
\toprule
...
%----------------------------------------------------------------------
\subsection{Pool Size Sensitivity (S7)}
\label{sec:exp_s7}
%----------------------------------------------------------------------

\begin{table}[t]
\centering
\caption{Pool size sweep on ML-1M. Coverage grows with pool size; NDCG and acceptance are stable.}
\label{tab:s7}
\small
\begin{tabular}{lcccc}
\toprule
...
\paragraph{Temperature Sensitivity (A1).}
...
\begin{table}[t]
\centering
\caption{Temperature sensitivity on ML-1M ($K$=3, $\tau$=0.35). Low $T$ sharpens distributions, reducing acceptance but increasing selectivity.}
\label{tab:a1}
\small
\begin{tabular}{lccc}
\toprule
...
\paragraph{Cold-Start vs Active Users (A2).}
...
\begin{table}[t]
\centering
\caption{User activity breakdown on ML-1M. Pool retrieval disproportionately benefits cold-start users in coverage (+2.6$\times$).}
\label{tab:a2}
\small
...
\paragraph{Latency Breakdown (A3).}
...
\paragraph{Cluster Count Sensitivity (A4).}
We vary the number of clusters $C \in \{10, 25, 50, 100\}$ on ML-1M.

\begin{table}[t]
\centering
\caption{Cluster count sensitivity on ML-1M. More clusters increase MCG and coverage but slightly reduce NDCG.}
\label{tab:a4}
\small
\begin{tabular}{lcccc}
\toprule
...
& Spec.\ (ours) & \textbf{.102} & 85\% & 1.5$\times$ & .027 \\
& Sp.+Pool (ours) & .093 & 84\% & 0.7$\times$ & \textbf{.062} \\
\bottomrule
\end{tabular}
\end{table}
...
On MIND-large (72K users $\times$ 21K articles after 5-core, 3 seeds), speculative serving achieves 83\% NDCG retention, 100\% acceptance, MCG=60\%, and 9.45$\times$ speedup; on Amazon Movies-and-TV (86K users $\times$ 33K items, very sparse) the pool variant lifts NDCG +136\% at 53\% acceptance. Replacing MF with LightGCN~\cite{lightgcn2020} (3 GCN layers) drops MCG to 23\% on ML-1M and 6\% on A-Electronics with $\sim$100\% acceptance: GNN-induced geometric concentration makes the nearest cluster nearly always sufficient, demonstrating the framework auto-adapts---homogeneous embeddings reduce to single-cluster acceptance while heterogeneous models exploit MCG.
...
Substituting MF item/user embeddings with Gemma-4-E2B-derived embeddings~\cite{gemma4_2025} (1536-dim mean-pool of titles, PCA-projected to 64-dim) feeds the same pipeline unchanged. On ML-1M (3 seeds), \emph{hybrid} mode (Gemma clusters + MF recs) achieves NDCG=0.145 (92.6\% retention) at 10.7\% acceptance---the score-ratio criterion is stricter in semantic space, yielding higher quality per accept. Critically, MCG rises from 51.9\% (MF) to 63.8\% (Gemma-4): richer semantic embeddings produce more diverse clusters, strengthening the multi-cluster gain property. The pipeline requires \emph{no code changes}.

\paragraph{Competitor Baselines.}
We compare against four systems spanning retrieval, two-tower, and speculative-decoding paradigms. \emph{Two-tower retrieve-and-rerank}: a lightweight MF ($d$=16) retrieves top-100 candidates, re-ranked by the full MF ($d$=64). On ML-1M, TwoTower-16 achieves NDCG=0.137 (87\% retention) at 8.8$\times$ speedup---\emph{faster and higher quality} than Spec~K=3 (0.102, 5.4$\times$). On Amazon~Electronics (sparse), the gap narrows: TwoTower-16 NDCG=0.008 (55\% retention) versus Spec~K=3's 0.012 (80\%)---speculative serving preserves more quality on sparse data because the acceptance criterion uses the full bias-aware score function, while the small retrieval model's bias terms are less accurate. \emph{FAISS retrieval}: on ML-1M, FAISS Flat is faster (0.07ms) with comparable NDCG (0.100 vs 0.102); on Electronics, Spec wins on quality (0.012 vs 0.005). \emph{BiLD-style draft}~\cite{leviathan2023}: 55\% accept vs RecCache's 91\%---1.7$\times$ more throughput without extra training. \emph{LASER-relaxed}~\cite{laser2025}: mean ratio accepts 100\% (NDCG=0.105) vs product rule's 91\% (NDCG=0.107)---stricter selection marginally improves quality.

\paragraph{Statistical Significance.}
Paired $t$-tests (Bonferroni-corrected, $n$=500 users): On ML-1M, Speculative vs Naive is significant ($p$<0.001), confirming the score-ratio criterion improves over blind caching. On Electronics, Speculative vs Fresh is \emph{not} significant ($p$=0.11), confirming quality is preserved on sparse data. On MIND, Spec+Pool vs Fresh is not significant ($p$=1.0).

\paragraph{Online Serving Replay.}
We replay 1,188 ML-1M test requests in temporal order, comparing six serving policies: Fresh, Popularity (global top-$N$), FAISS, Naive cache, Spec~K=3, and Spec+Pool. Beyond NDCG, we report CTR proxy (fraction of recs matching held-out test interactions---a standard offline evaluation protocol for replay-based serving~\cite{he2017ncf}) and novelty ($-\log_2 \text{pop}$). Popularity wins on CTR (0.169) due to ML-1M's strong popularity bias but has the lowest novelty (8.6); Fresh NDCG=0.159, CTR=0.145, novelty=10.1. Spec~K=3 (0.100, 0.094, 9.7) is comparable to FAISS (0.104, 0.099, 10.6) and Naive (0.099, 0.091, 9.7). Rolling 200-request windows show no temporal drift. Under 3 random orderings, all metrics are order-invariant ($\sigma_{\text{NDCG}}{=}0.001$).
...
Six ranking metrics on ML-1M (3 seeds, Spec K=3 retention vs Fresh): NDCG 65\%, Recall@10 62\%, HR@10 76\%, MRR 67\%, MAP 68\%, ILD 98\%---all dimensions degrade proportionally, not just NDCG. On Amazon Electronics: NDCG 80\%, Recall 79\%, HR 84\%, MRR 79\%, MAP 79\%, ILD 91\%.

\paragraph{Long-tail and Fairness.}
Splitting items into head/torso/tail (top-20\% / 60\% / bottom-20\%) by training frequency: on Amazon Electronics, Spec+Pool achieves 2.6$\times$ catalog coverage versus Fresh (3.8\%$\to$9.7\%) and reduces the recommendation-frequency Gini from 0.996 to 0.966; on ML-1M, pool retrieval raises Spec coverage from 1.8\% (static) to 6.2\%, a 3.4$\times$ increase within the caching framework. Per-cluster acceptance fairness (1500 ML-1M users) yields a Gini of 0.262 across 49 clusters---no cluster is starved.

\paragraph{Limitations.}
Speedup at small catalogs is modest (1.5--3.8$\times$) since MF inference is already $\sim$0.1ms; our latency-scaling experiment shows speedup grows to 27$\times$ at 100K items, and GPU generative recommenders with 5--50ms inference would gain proportionally more. Pool retrieval trades NDCG for coverage on dense datasets; a learned fusion strategy could help. The $\epsilon$-bound assumes stationarity; concept drift requires periodic re-clustering.

\paragraph{Hyperparameters and Reproducibility.}
MF with BPR loss~\cite{koren2009matrix} ($d$=64, 15 epochs, lr 0.001), online K-Means ($C$=50), score-ratio ($T$=1.0, $\tau$=0.35, $K$=3), pool size 200. Datasets: ML-1M (6.0K users / 3.7K items / 800K interactions / 3.57\% density); A-Elec.\ (17.9K / 9.8K / 119K / 0.07\%); A-Arts (10.1K / 7.8K / 57K / 0.07\%); MIND-small (29.4K / 15.1K / 200K / 0.05\%). CPU only (i9-14900HX, 64GB), $\sim$2 min train per dataset. All numbers are 3-seed averages.
...
We introduced speculative recommendation serving, establishing a principled connection between speculative decoding and recommendation caching. The score-ratio acceptance criterion enables multi-cluster speculation, recovering 43--64\% additional cache hits from non-nearest clusters---a consistent empirical pattern supported across MF, NeuMF, LightGCN, and Gemma-4 backbones on six datasets (up to 86K users). Embedding pool retrieval increases catalog coverage up to 2.6$\times$ on sparse data with reduced recommendation-frequency Gini. In controlled comparisons, the criterion is competitive with FAISS retrieval, and in some sparse settings achieves higher quality, while requiring no learned policies. The empirically supported $\epsilon$-bound (Spearman $\rho=-0.655$) provides a principled characterization of the quality--speed tradeoff. Speculative latency is near-constant, with speedup scaling linearly in catalog size---a property most valuable for large-catalog production systems. Unlike learned caching policies, our framework is training-free and directly applicable to existing recommendation systems.
